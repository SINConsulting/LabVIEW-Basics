...
int LinxESP8266WifiListener::SetPort(unsigned short port)
{
	LinxWifiPort = port;
	return L_OK;
}
...
int LinxESP8266WifiListener::PrintWifiInfo()
{
	...
	szPassPhrase = LinxWifiPw;  
  	...
  	//Used to check the IP address
  	Serial.println("");
	Serial.println("");
  	Serial.print("IP Address : "); Serial.println(IPAddress(WiFi.localIP()));
	Serial.print("Port       : "); Serial.println(LinxWifiPort);
	...
	LinxDev->DebugPrint(IpAddress[0], DEC);
	LinxDev->DebugPrint(".");
	LinxDev->DebugPrint(IpAddress[1], DEC);
	LinxDev->DebugPrint(".");
	LinxDev->DebugPrint(IpAddress[2], DEC);
	LinxDev->DebugPrint(".");
	LinxDev->DebugPrintln(IpAddress[3], DEC);
